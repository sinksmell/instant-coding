...
- ✅ **SDK 反转（2026-04-28）**：agent-runtime 从直 spawn `claude -p` 切到 `@anthropic-ai/claude-agent-sdk` 的 `query()`。新增：流式 content_block_delta → typewriter；SDK `canUseTool` hook → 交互式权限往返；读 `~/.claude/projects/*.jsonl` 回放历史（`/agent/history` + localStorage sessionId）；subagent 视觉嵌套（`parentToolUseId` → 缩进 + 左边框）。详见 ARCHITECTURE §2.2 / §4.3 / §5.3 / §5.8
- 🟡 **M9**：文件树 + 内嵌编辑 —— agent-runtime `/fs/{tree,read,write}`（单文件最大 2MB、内置忽略 `node_modules` 等 15 个目录、NUL 字节检测 binary；smoke 覆盖 tree/read/write/binary/sandbox/413/nested mkdir，全绿）；共享路径守卫 `src/safe-path.js` 同时服务 `/git/worktree`；BFF `app/api/agent/fs/[taskId]/[...path]` 透明 HTTP 代理；前端 `components/files-panel.tsx` Monaco 编辑器 + 懒展开树 + Cmd/Ctrl+S 保存 + binary 提示 + 脏态标志；`/chat/[id]` 右栏升级为 Diff/Files/Shell 三 tab。ARCHITECTURE §5.7 新增 fs REST 规格。文件系统 watcher 推 delta 作为 tech debt（刷新按钮先用）
