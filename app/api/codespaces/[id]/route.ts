...
import { deleteCodespace } from "@/lib/github/codespaces"
...
export async function DELETE(
  ...
  try {
    ...
    await deleteCodespace(octokit, cs.codespace_name)

    await supabase.from("codespaces").delete().eq("id", params.id)

    return NextResponse.json({ success: true })
  } catch (error) {
    const message = error instanceof Error ? error.message : "Failed to delete codespace"
